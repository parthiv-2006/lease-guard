...
| 1.4 | Re-seed RTA provisions whose nested sub-items were dropped | `scripts/build_corpus.py` | ❌ Open (found 2026-09-22). ~10 rows end mid-list at "if,", "that," or "the following:" — e.g. s.27(1) para 4 (inspection conditions i/ii), s.6.1(3), s.72(2), s.120.1(3), s.126(13), s.129, s.234. The parser keeps numbered paragraphs but drops roman-numeral / lettered children. Tenant checkers cite only the parent provisions, so they're unaffected; RAG answers about these provisions are incomplete. Re-run validate_retrieval.py after re-seeding. |
...
| 9.1 | Deterministic tenant checkers + letters with corpus-verified citations | `lib/*-checker.ts`, `lib/tenant-letters/` | ✅ 5 checkers (rent increase, eviction notice, deposit & fees, maintenance, landlord entry) + 5 letter templates. Every citation checked against the `statutes` table and listed in each file header; every template has a "never says illegal" test. (2026-09-22) |
