// LeaseGuard — Overview Panel

const { useState } = React;

function OverviewPanel({ report, onNavigate }) {
  const { lease, overall, clauses } = report;

  const riskLevelLabel = { critical: "Critical", high: "High", medium: "Medium", low: "Low" };
  ...
  return (
    <div>
      ...
      <SectionHeader
        title="Overview"
        subtitle={`Analysis of ${lease.address}, ${lease.city}  ·  ${lease.term}`}
      />

      {/* Main grid: gauge + summary */}
      <div style={{
        display: "grid", gridTemplateColumns: "200px 1fr", gap: "32px",
        marginBottom: "32px", alignItems: "start",
      }}>
        {/* Arc gauge */}
        <div style={{
          background: "#fff", border: "1px solid #e8e4dc", borderRadius: "10px",
          padding: "28px 20px", textAlign: "center",
          display: "flex", flexDirection: "column", alignItems: "center", gap: "12px",
        }}>
          <RiskArc score={overall.risk_score} size={140} />
          <div style={{
            fontSize: "11px", color: "#9a9590", letterSpacing: "0.06em",
            textTransform: "uppercase", fontFamily: "'DM Sans', sans-serif",
          }}>
            Overall Risk
          </div>
          <div style={{
            fontSize: "11px", color: "#b0aaa4", fontFamily: "'DM Sans', sans-serif",
            textAlign: "center", lineHeight: 1.4,
          }}>
            Corpus: {overall.corpus_version}<br />
        ...
        {/* Executive summary */}
        <div style={{
          background: "#fff", border: "1px solid #e8e4dc", borderRadius: "10px", padding: "28px",
        }}>
          <div style={{
            fontSize: "11px", letterSpacing: "0.08em", textTransform: "uppercase",
            color: "#9a9590", fontFamily: "'DM Sans', sans-serif", fontWeight: 500,
            marginBottom: "12px",
          }}>Executive Summary</div>
          <p style={{
            margin: "0 0 20px", fontSize: "14px", color: "#181614",
            lineHeight: 1.7, fontFamily: "'DM Sans', sans-serif",
          }}>
            {overall.executive_summary}
          </p>
          <div style={{
            fontSize: "11px", color: "#9a9590", fontFamily: "'DM Sans', sans-serif",
      ...
      <div style={{
        ...
        <div style={{
          padding: "16px 24px", borderBottom: "1px solid #e8e4dc",
          display: "flex", alignItems: "center", justifyContent: "space-between",
        }}>
          <span style={{
            fontSize: "13px", fontWeight: 500, color: "#181614",
            fontFamily: "'DM Sans', sans-serif",
          }}>
            Clause Breakdown — {overall.clause_count} clauses analysed
          </span>
          <button onClick={() => onNavigate("clauses")} style={{
            fontSize: "12px", color: "#6b6560", background: "none",
        ...
        <div style={{ padding: "20px 24px 12px" }}>
          <div style={{ display: "flex", gap: "2px", height: "8px", borderRadius: "4px", overflow: "hidden", marginBottom: "10px" }}>
            {["critical", "high", "medium", "low"].map(level => {
              ...
              return pct > 0 ? (
                <div key={level} style={{
                  flex: pct, background: riskColor(level), transition: "flex 0.4s",
                  minWidth: 0,
                }} title={`${count} ${level}`} />
              ) : null;
            })}
            ...
            {["critical", "high", "medium", "low"].map(level => {
              ...
              return (
                <div key={level} style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                  <span style={{ width: 8, height: 8, borderRadius: "2px", background: riskColor(level), display: "inline-block" }}></span>
                  <span style={{ fontSize: "12px", color: "#6b6560", fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div>
          {clauses.map((clause, i) => (
            <div key={clause.id} onClick={() => onNavigate("clauses")} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: "11px", color: "#9a9590", marginTop: "2px" }}>
                  {CLAUSE_TYPE_LABELS[clause.primary_type]}
                  {clause.is_potentially_unenforceable && (
                    <span style={{ color: "#b91c1c", marginLeft: "8px" }}>· Potentially unenforceable</span>
                  )}
                </div>
              </div>
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: "13px", color: "#6b6560", lineHeight: 1.5 }}>
              {report.negotiation_points.filter(n => n.walk_away_threshold).map(n => n.clause_label).join(" and ")} are flagged
              as potential walk-away concerns. If the landlord refuses to remove or substantially revise these clauses,
              consider whether this tenancy is right for you.{" "}
              <button onClick={() => onNavigate("negotiation")} style={{
      ...
      <div style={{
        ...
        fontSize: "11px", color: "#9a9590", lineHeight: 1.5,
        fontFamily: "'DM Sans', sans-serif",
      }}>
        ...
        Every legal claim in this report is grounded in retrieved statute text (corpus version {overall.corpus_version}, updated {overall.corpus_date}).
        For professional legal judgment, consult a licensed paralegal or lawyer. Community Legal Clinics in Ontario offer free tenant legal help.
      </div>
    </div>
  );
}
