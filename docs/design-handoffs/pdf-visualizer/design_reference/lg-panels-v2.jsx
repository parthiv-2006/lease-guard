// LeaseGuard — Report Panels (v2 — with clause activation events)
// Adds: lg-clause-activate CustomEvent dispatched when any clause card opens.
// RedFlagsPanel, ClauseExplorerPanel, NegotiationPanel,
// MissingPanel, ContradictionsPanel, SourcesPanel, AgentTracePanel

const { useState, useMemo, useEffect } = React;

// ── Helper: fire clause activation ──────────────────────────────────────────
function activateClause(id) {
  if (!id) return;
  window.dispatchEvent(new CustomEvent('lg-clause-activate', { detail: id }));
}
...
function ClauseCard({ clause, negotiation, defaultOpen }) {
  ...
  // Fire activation on mount when defaultOpen=true (first red flag)
  useEffect(() => {
    if (defaultOpen) activateClause(clause.id);
  }, []);

  function handleToggle() {
    const next = !open;
    setOpen(next);
    if (next) activateClause(clause.id);
  }
    ...
    <div style={{
      ...
      <button onClick={handleToggle} style={{
        ...
        <div style={{
          ...
          fontSize: "10px", color: "#b0aaa4",
          fontFamily: "'DM Sans', sans-serif",
          flexShrink: 0, whiteSpace: "nowrap",
        }}>
          <svg width="11" height="11" viewBox="0 0 16 16" fill="none" stroke="#c5bfb5" strokeWidth="1.5">
            <rect x="1" y="2" width="6" height="12" rx="1" />
            <rect x="9" y="2" width="6" height="12" rx="1" />
          </svg>
          <span>p.{({ c1:2, c2:2, c3:3, c4:3, c5:4, c6:5, c7:6 })[clause.id] || '—'}</span>
        </div>
...
function NegotiationPanel({ report }) {
  ...
  function NegotiationCard({ n }) {
    ...
    function handleToggle() {
      const next = !open;
      setOpen(next);
      if (next && n.clause_id) activateClause(n.clause_id);
    }
            ...
            <div style={{ padding: "18px 20px", borderBottom: "1px solid #f0ede6" }}>
              <div style={{ display: "flex", gap: "12px", alignItems: "flex-start", marginBottom: "12px" }}>
                <div style={{ flex: 1 }}>
                  ...
                  {/* Conflicting clauses — clickable to activate in PDF */}
                  <div style={{ display: "flex", gap: "8px", alignItems: "center", flexWrap: "wrap" }}>
                    <button
                      onClick={() => activateClause(x.clause_a_id)}
                      style={{
                    ...
                    <button
                      onClick={() => activateClause(x.clause_b_id)}
                      style={{
                        fontSize: "12px", padding: "3px 10px", background: "#f6f3ee",
                        border: "1px solid #e8e4dc", borderRadius: "4px", color: "#5c5751",
                        cursor: "pointer", transition: "border-color 0.15s",
            ...
            <div style={{ display: "flex", gap: "14px", alignItems: "flex-start" }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", gap: "16px", alignItems: "center", flexWrap: "wrap" }}>
                  ...
                  <span style={{ fontSize: "11px", color: "#9a9590" }}>{s.corpus_version}</span>
                    ...
                    <span style={{ fontSize: "11px", color: "#9a9590" }}>
                      ...
                      {s.relevant_clauses.map((id, i) => {
                        ...
                        return (
                          <button
                            key={id}
                            onClick={() => activateClause(id)}
                            style={{
                              background: "none", border: "none", padding: "0 2px",
                              cursor: "pointer", fontSize: "11px", color: "#1d4ed8",
...
function AgentTracePanel({ report }) {
  ...
  const toolColors = {
    parse_document: "#1d4ed8", detect_jurisdiction: "#7c3aed", segment_clauses: "#0369a1",
    classify_clause: "#0d9488", lookup_statute: "#b45309", lookup_tribunal: "#9333ea",
    score_risk: "#c2410c", detect_contradiction: "#b91c1c", check_missing: "#15803d",
    generate_negotiation: "#1d4ed8", generate_report: "#374151",
  };
            ...
            <div key={step.id} style={{ display: "flex", gap: "0" }}>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: "36px", flexShrink: 0 }}>
                ...
                <div style={{
                  ...
                  <span style={{ fontSize: "9px", fontWeight: 700, color: "#fff", fontFamily: "'DM Sans', sans-serif" }}>
                    {step.sequence}
                  </span>
                </div>
              ...
              <div style={{ flex: 1, paddingLeft: "12px", paddingBottom: "8px" }}>
                <div style={{ background: "#fff", border: "1px solid #e8e4dc", borderRadius: "8px", overflow: "hidden", marginBottom: "2px" }}>
                  <button onClick={() => setExpanded(isOpen ? null : step.id)} style={{
                    ...
                    <code style={{ fontSize: "12px", fontFamily: "'JetBrains Mono', monospace", color: col, fontWeight: 500, flex: 1, textAlign: "left" }}>
                      {step.tool_name}
                    </code>
                    <span style={{ fontSize: "11px", color: "#9a9590" }}>
                  ...
                  {isOpen && (
                    <div style={{ padding: "14px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                      {["Input", "Output"].map((label, idx) => (
                        <div key={label}>
                          <div style={{ fontSize: "10px", letterSpacing: "0.07em", textTransform: "uppercase", color: "#9a9590", fontWeight: 500, marginBottom: "6px" }}>{label}</div>
                          <pre style={{
                            margin: 0, fontSize: "11px", fontFamily: "'JetBrains Mono', monospace",
                            color: idx === 0 ? "#5c5751" : "#181614", background: "#f6f3ee",
                            padding: "10px", borderRadius: "5px", overflow: "auto",
                            whiteSpace: "pre-wrap", wordBreak: "break-word",
                          }}>
                            {JSON.stringify(idx === 0 ? step.input_summary : step.output_summary, null, 2)}
                          </pre>
                        </div>
                      ))}
                    </div>
